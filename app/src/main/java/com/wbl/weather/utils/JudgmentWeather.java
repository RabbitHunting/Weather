...
public class JudgmentWeather {
    ...
    //背景
    public static int backid(String id) {
     int backid;
        if (id != null) {
            boolean back = EasyDate.dayAndNight();
            if (back) {
                //晚上
               backid = getNightBack(id);
               return backid;
            } else {
                //白天
                backid = getDayBack(id);
                return backid;
            }
        } else {
            return R.color.qtransluncent;
        }
    }

    //天气图标
    public static int weatherIcon(String id,String time) {
        int icon;
        if (id !=null) {
            boolean back = EasyDate.dayandnight(time);
            if (back) {
                //晚上
               icon = getNightIconDark(id);
               return icon;
            } else {
                //白天
                icon = getDayIconDark(id);
                return icon;
            }
        } else {
            return 1;
     ...
     * 获取白天天气图标
     */
    private static int getDayIconDark(String weather) {
        int imageId;
        switch (weather) {
            case "100":
     ...
     * 获取晚上天气图标
     */
    private static int getNightIconDark(String weather) {
        int imageId;
        switch (weather) {
            case "150":
     ...
     * 获取白天背景
     */
    private static int getDayBack(String weather) {
        int imageId;
        switch (weather) {
            case "100":
     ...
     * 获取晚上背景
     */
    private static int getNightBack(String weather) {
        int imageId;
        switch (weather) {
            case "100":
